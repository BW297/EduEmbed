...
parser.add_argument('--method', default='kancd', type=str, help='prediction method', required=True)
parser.add_argument('--train_file', default='data/SLP-BIO,data/SLP-PHY', type=str, help='train file list', required=True)
parser.add_argument('--test_file', default='data/SLP-MAT', type=str, help='test file list', required=True)
...
parser.add_argument('--lr', type=float, help='learning rate', default=2.5e-4, required=True)
parser.add_argument('--dropout', type=float, help='dropout', default=0.5, required=False)
parser.add_argument('--inter', type=str, help='orcdf interfunction', default='kancd', required=False)
...
set_seed(seed)
wandb.init(project="CrossCD", name=f"{train_file}--{test_file}--{method}--{batch_size}--{lr}--{seed}", config=config_dict)
pprint(config_dict)
