...
def get_mlp(input_channel, output_channel, dropout, dtype, device):
...
class ORCDF_Extractor(nn.Module):
    def __init__(self, dataset, **dict):
        ...
        self.__student_emb = get_mlp(self.text_dim, self.latent_dim, self.dropout, self.dtype, self.device)
        self.__knowledge_emb = get_mlp(self.text_dim, self.latent_dim, self.dropout, self.dtype, self.device)
        self.__exercise_emb = get_mlp(self.text_dim, self.latent_dim, self.dropout, self.dtype, self.device)
        self.__disc_emb = get_mlp(self.text_dim, 1, self.dropout, self.dtype, self.device)
