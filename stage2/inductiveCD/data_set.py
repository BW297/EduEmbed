...
class DATA_SET:
    def __init__(self, train_list, test_list, model_type) -> None:
        ...
        for index, file in enumerate(self.test_file):
            if index == 0:
                with open(file + "/data.json", 'r') as json_file:
                    json_f = json.load(json_file)
                add_stu_num, add_exer_num = json_f["student_num"], json_f["exercise_num"]
                df = pd.read_csv(file + "/induct_train.csv", header=None)
                dfs.append(df)
            else:
                df = pd.read_csv(file + "/induct_train.csv", header=None)
                df.iloc[:, 0] += add_stu_num
    ...
    def get_id_data(self,dtype,device):
        stu_n, exer_n, test_stu_n, test_exer_n, _, _ = self.get_num()
        train_r_matrix = torch.zeros(stu_n, exer_n,dtype=dtype,device=device)
        train_data = self.get_id_response()
        for _, row in train_data.iterrows():
            ...
            if int(score)==0:
                score=-1
            train_r_matrix[int(student_id), int(exercise_id)] = int(score)
        
        train_dict={
            'student': train_r_matrix,
            'exercise': train_r_matrix.T,
            "disc": train_r_matrix.T
        }
        return train_dict
    ...
    def get_text_embedding(self, dtype, device):
        ...
        for file in self.train_file:
            know_emb = pd.read_csv(file + "/"+self.model_type+"/know_embedding.csv", header=None)
            knowledge_emb.append(torch.tensor(know_emb.values, dtype=dtype, device=device))
            stu_emb = pd.read_csv(file + "/"+self.model_type+"/stu_embedding.csv", header=None)
            student_emb.append(torch.tensor(stu_emb.values, dtype=dtype, device=device))
            exer_emb = pd.read_csv(file +"/"+self.model_type+ "/exer_embedding.csv", header=None)
    ...
    def get_orginal_iter(self,batch_size):
        _,_,_,_,train_know_n,_=self.get_num()
        _,test_q_matrix=self.get_q_matrix()
        q_matrix=test_q_matrix[:,train_know_n:]
        train_data, test_data=self.get_orginal_data()
        user=train_data.iloc[:,0]
        item=train_data.iloc[:,1]
        score=train_data.iloc[:,2]
        train_dataset=TensorDataset( torch.tensor(user.apply(int), dtype=torch.int64),
            torch.tensor(item.apply(int), dtype=torch.int64),
        ...
        item=test_data.iloc[:,1]
        score=test_data.iloc[:,2]
        test_dataset=TensorDataset( torch.tensor(user.apply(int), dtype=torch.int64),
            torch.tensor(item.apply(int), dtype=torch.int64),
            q_matrix[np.array(item, dtype=int), :],
            torch.tensor(score, dtype=torch.float64))
        return DataLoader(train_dataset,batch_size=batch_size,shuffle=True),DataLoader(test_dataset,batch_size=batch_size,shuffle=False)