...
def get_mlp(input_channel, output_channel):
...
class IRT(nn.Module):
    def __init__(self, config, num_students, num_questions, num_dim):
        super().__init__()
        self.config = config
        ...
        self.LLM_transfer_stu = get_mlp(self.emb_num, self.num_dim)
        self.LLM_transfer_k_difficulty = get_mlp(self.emb_num, self.num_dim)
        self.LLM_transfer_e_discrimination = get_mlp(self.emb_num, 1)
    ...
    def forward(self, student_ids, question_ids):
        or_theta = self.theta(student_ids)
        or_alpha = self.alpha(question_ids)
        or_beta = self.beta(question_ids)

        theta = self.LLM_transfer_stu(self.init_stu[student_ids])
        alpha = self.LLM_transfer_k_difficulty(self.init_exer[question_ids])
        beta = self.LLM_transfer_e_discrimination(self.init_exer[question_ids])

        theta=or_theta
        alpha=self.lam*alpha+(1-self.lam)*or_alpha
        beta=self.lam*beta+(1-self.lam)*or_beta
...
class IRTModel(AbstractModel):
    ...
    def init_model(self, data: Dataset):
        policy_lr=0.0005
        self.model = IRT(self.config, data.num_students, data.num_questions, self.config['num_dim'])
    ...
    def train(self, train_data: TrainDataset, log_step=1):
        ...
        for ep in range(1, epochs + 1):
            loss = 0.0
            epoch_losses = []
            for cnt, (student_ids, question_ids, _, labels) in enumerate(tqdm(train_loader, "Epoch %s" % ep)):
    ...
    def get_alpha(self, question_id):
        alpha = self.model.LLM_transfer_k_difficulty(self.model.init_exer).detach()
        or_alpha=self.model.alpha.weight.data
        true=self.lam*alpha+(1-self.lam)*or_alpha
        ...
        """
        return true.cpu().numpy()[question_id]
    ...
    def cal_loss(self,student_id,exercise_id):
        or_theta = self.model.theta(student_id)
        or_alpha = self.model.alpha(exercise_id)
        or_beta = self.model.beta(exercise_id)
        theta = torch.sigmoid(self.model.LLM_transfer_stu(self.model.init_stu[student_id]))
        alpha = torch.sigmoid(self.model.LLM_transfer_k_difficulty(self.model.init_exer[exercise_id]))
        beta = torch.sigmoid(self.model.LLM_transfer_e_discrimination(self.model.init_exer[exercise_id]))
        data={
            "theta":[theta,or_theta],
            "alpha":[alpha,or_alpha],
            "beta":[beta,or_beta]
        }
        loss=0
        loss+=self.con_loss(data['alpha'])
        loss+=self.con_loss(data['beta'])
        return loss*self.alp
