...
class NCAT(nn.Module):
    def __init__(self, n_question, d_model=10, n_blocks=1,kq_same=True, dropout=0.0, policy_fc_dim=512, n_heads=1, d_ff=2048,  l2=1e-5, separate_qa=None, pad=0, device='cpu'):
        super().__init__()
        """
        Input:
...
class env:
    def __init__(self,data,concept_map,config,T):
        ...
        pth_path=f"../model/ckpt/{config['dataset']}_{self.CDM}.pt"
        # print(pth_path)
        self.model, self.dataset = self.load_CDM(self.CDM,data,pth_path,config)
        #print(self.model)
    ...
    def candidate_items(self):
        return set(self.tsdata.candidate[self.state[0][0]])
    ...
    def load_CDM(self,name,data,pth_path,config):
        if name == 'irt':
            ...
            model = RCDModel(**config)
        elif name == 'orcdf':
            model = ORCDFModel(**config)
        else:
            model = GrpahModel(**config)
        model.init_model(data)
        model.adaptest_load(pth_path)
        return model ,data.data
    
    def step(self, action,sid):
        assert action in self.tsdata.candidate[self.state[0][0]] and action not in self.short
...
class NCATModel():
    ...
    def __init__(self, NCATdata,concept_map,config,test_length):
        ...
        self.device = config['device']
        self.fa = NCAT(n_question=NCATdata.num_questions+1,device=self.device).to(self.device)
    ...
    def ncat_policy(self,sid,THRESHOLD,used_actions,type,epoch):
        ...
        if type == "training":
            if len(self.memory) >= self.config['batch_size']:
                ...
                batch = [self.memory[item] for item in np.random.choice(range(len(self.memory)),(self.config['batch_size'],))]
                data = self.convert_batch2dict(batch,epoch)
                loss = self.fa.optimize_model(data, 0.001)
                self.tau += 5
        return 
    ...
    def convert_batch2dict(self, batch, epoch):
        ...
        value[:,0] = -500
        goals = np.max(value,axis=-1)*np.asarray(dones)*min(0.8,decay_function(max(self.config['end']-epoch,0)+1,self.config)) + goals
