...
if __name__ == '__main__':
    ...
    # read datasets
    response_logs = pd.read_csv(f'../../datasets/{dataset}/response.csv', header=None)
    q = pd.read_csv(f'../../datasets/{dataset}/q_matrix.csv', header=None)
    config['LLM_stu_emb'] = pd.read_csv(f"../../datasets/{dataset}/{config['LLM_model_name']}/stu_embedding.csv", header=None).to_numpy()
    config['LLM_exer_emb'] = pd.read_csv(f"../../datasets/{dataset}/{config['LLM_model_name']}/exer_embedding.csv", header=None).to_numpy()
    config['LLM_know_emb'] = pd.read_csv(f"../../datasets/{dataset}/{config['LLM_model_name']}/know_embedding.csv", header=None).to_numpy()
    ...
    pre_train_stu = [stu for stu in total_stu if stu not in train_stu] # 剩余0.2
    train_triplets = []
    pre_train_triplets = []
    concept_map = {}
    cnt = 0
    ...
    for question in q.values:
        concept_map[cnt] = np.where(np.array(question))[0].tolist()  # 相关的kc编号
        cnt += 1
