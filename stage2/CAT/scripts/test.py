...
from model.utils import get_r_matrix
...
parser.add_argument('--dataset', default='EDM-CUP-23', type=str, help='benchmark')
...
parser.add_argument('--LLM_model_name', default='bert', type=str)
parser.add_argument('--lamda',type=float,default=0.75)
...
if __name__ == '__main__':
    ...
    config = {
        ...
        'policy':'no-bobcat',
        'betas': (0.9, 0.999),
        'policy_path': '../model/ckpt/biirt_policy.pt',
        # for NCAT
        'THRESHOLD' :300,
        'start':0,
        'end':3000,
        ...
        'num_layers': 2,
        'weight_reg': 0,
        'LLM_model_name': parser.LLM_model_name,
        'lamda':parser.lamda,
        'alpha':parser.alpha
    }
    name = f"{config['cdm']}-{config['dataset']}-{config['strategy']}-seed{config['seed']}"
    if parser.wandb:
        run = wb.init(project="CAT", name=name,
                    config=config)
    if parser.strategy == 'BOBCAT':
        config['policy'] = 'bobcat'
        if parser.cdm == 'irt':
            config['policy_path'] = '../model/ckpt/biirt_policy.pt'
        elif parser.cdm == 'ncd':
            config['policy_path'] = '../model/ckpt/binn_policy.pt'
    # fixed test length
    ...
    # read datasets
    response_logs = pd.read_csv(f"../../datasets/{config['dataset']}/response.csv", header=None)
    if 'SLP' in config['dataset']:
        response_logs = response_logs.groupby(0).filter(lambda x: len(x) > 15)
    q = pd.read_csv(f"../../datasets/{config['dataset']}/q_matrix.csv", header=None)
    config['LLM_stu_emb'] = pd.read_csv(f"../../datasets/{config['dataset']}/{config['LLM_model_name']}/stu_embedding.csv", header=None).to_numpy()
    config['LLM_exer_emb'] = pd.read_csv(f"../../datasets/{config['dataset']}/{config['LLM_model_name']}/exer_embedding.csv", header=None).to_numpy()
    config['LLM_know_emb'] = pd.read_csv(f"../../datasets/{config['dataset']}/{config['LLM_model_name']}/know_embedding.csv", header=None).to_numpy()
    ...
    config['train_triplets'] = np.array(train_triplets)
    config['r'] = get_r_matrix(np.array(train_triplets), config['stu_num'], config['prob_num'])
