...
def get_top_k_concepts(datatype: str, topk: int = 10):
    q = pd.read_csv('../../datasets/{}/q_matrix.csv'.format(datatype), header=None).to_numpy()
    a = pd.read_csv('../../datasets/{}/{}response.csv'.format(datatype, datatype), header=None).to_numpy()
    skill_dict = {}
...
def get_average_correct_rate(datatype: str):
    data = pd.read_csv('../../datasets/{}/{}response.csv'.format(datatype, datatype), header=None).to_numpy()
    cdata = []
    right = 0
...
def get_group_by_correct_rate(datatype: str):
    ...
    with open('../../datasets/{}/{}high.pkl'.format(datatype, datatype), 'wb') as f:
        pickle.dump(high_indices, f)
    with open('../../datasets/{}/{}middle.pkl'.format(datatype, datatype), 'wb') as f:
        pickle.dump(middle_indices, f)
    with open('../../datasets/{}/{}low.pkl'.format(datatype, datatype), 'wb') as f:
